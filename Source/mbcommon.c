...
bool CovertSingleCommandCoilToBoolStatus(uint16_t coilValue, uint16_t *value)
{
    bool state = false;
...
/*获取想要读取的Coil量的值*/
void GetCoilStatus(uint16_t startAddress, uint16_t quantity, uint16_t *statusList)
{
    // 如果需要Modbus TCP Server/RTU Slave应用中实现具体内容
}

/*获取想要读取的InputStatus量的值*/
void GetInputStatus(uint16_t startAddress, uint16_t quantity, uint16_t *statusValue)
{
    // 如果需要Modbus TCP Server/RTU Slave应用中实现具体内容
}
...
/*设置单个线圈的值*/
void SetSingleCoil(uint16_t coilAddress, uint16_t coilValue)
{
    // 如果需要Modbus TCP Server/RTU Slave应用中实现具体内容
}
...
/*设置多个线圈的值*/
void SetMultipleCoil(uint16_t startAddress, uint16_t quantity, uint16_t *statusValue)
{
    // 如果需要Modbus TCP Server/RTU Slave应用中实现具体内容
}
...
/*更新读回来的线圈状态*/
void UpdateCoilStatus(uint8_t salveAddress, uint16_t startAddress, uint16_t quantity, uint16_t *stateValue)
{
    // 在客户端（主站）应用中实现
}

/*更新读回来的输入状态值*/
void UpdateInputStatus(uint8_t salveAddress, uint16_t startAddress, uint16_t quantity, uint16_t *stateValue)
{
    // 在客户端（主站）应用中实现
}
