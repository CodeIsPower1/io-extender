...
{
    bool checkResult;
    checkResult = (GenerateCRC16CheckCode(message, length - 2) == GET_CRC16_DATA(message, length)) ? true : false;
    return checkResult;
}
...
/*生成从站应答主站的响应*/
uint16_t SyntheticSlaveAccessRespond(uint8_t *receivedMessage, uint16_t *valueList, uint8_t *respondBytes)
{
    uint16_t respondLength = 0;

    respondLength = GenerateMasterAccessRespond(receivedMessage, valueList, respondBytes);
