...
static uint16_t ConvertBoolArrayToMBByteArray(uint16_t *sData, uint16_t length, uint8_t *oData);
...
static void ConvertMBByteArrayTotBoolArray(uint8_t *sData, uint16_t *oData);
...
uint16_t GenerateMasterAccessRespond(uint8_t *receivedMessage, uint16_t *valueList, uint8_t *respondBytes)
{
    uint16_t index = 0;
    uint16_t bytesCount;
    uint16_t addrQty=0;
    ...
    if (CheckFunctionCode(functionCode) != Modbus_OK)
    {
        ...
        respondBytes[index++] = functionCode | 0x80; // 功能码
        return index;
    }

    respondBytes[index++] = *receivedMessage;       // 从站地址
    respondBytes[index++] = functionCode; // 功能码

    /*读线圈或状态量*/
    if (((functionCode == ReadCoilStatus) || (functionCode == ReadInputStatus))  &&  (valueList))
    {
        addrQty = (receivedMessage[4]<<8)|(receivedMessage[5]);
        index++;
        bytesCount = ConvertBoolArrayToMBByteArray(valueList, addrQty, respondBytes+index);
        respondBytes[2] = bytesCount; // 字节数
        index += bytesCount;
    }

    /*读寄存器数据*/
    if (((functionCode == ReadHoldingRegister) || (functionCode == ReadInputRegister)) && (valueList))
    {
        addrQty = (receivedMessage[4]<<8)|(receivedMessage[5]);
        index++;
        bytesCount = ConvertRegisterArrayToMBByteArray(valueList, addrQty, respondBytes+index);
        respondBytes[2] = bytesCount; // 字节数量
        index += bytesCount;
    }
    ...
    if (functionCode > ReadInputRegister)
    {
        ...
        /*写多个线圈操作*/
        if ((functionCode == WriteMultipleCoil) && (valueList))
        {
            ConvertMBByteArrayTotBoolArray(receivedMessage, valueList);
        }

        /*写多个寄存器*/
        if ((functionCode == WriteMultipleRegister) && (valueList))
        {
            ConvertMBByteArrayToRegisterArray(receivedMessage, valueList);
        }
    }
